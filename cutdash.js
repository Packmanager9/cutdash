...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let serverID = Math.random()

    ships = []


    class LineOPGrad {
        constructor(object, target,color1, color2, width) {
            this.object = object
            this.target = target
            this.color2 = color2
            this.color1 = color1
        ...
        draw() {
            ...
            grad.addColorStop(.10, this.color1+'88')
            grad.addColorStop(.90, this.color2+'88')
    ...
    class World {
        constructor(){
            this.body = new CircleRing(0,0, 1440, "#FFFFFF")
        }
        draw(){
            this.body.draw()
        }
    ...
    class Ship{
        constructor(){
            this.body = new Circle(0,0, 10, getRandomColor())
            this.serverID = serverID
            this.link = new LineOP(this.body, zehwarldo.body)
            this.links = []
            this.speed = 4
            ...
            if(ships.length > 0){
                ...
                for(let t = 1;t<ships.length;t++){
                    let link = new LineOPGrad(this.body, ships[t].body, this.body.color, ships[t].body.color, 7)
                    this.links.push(link)
                    // ships[t].links.push(link)
                }
            }
            ships.push(this)
        }
        ai(){
            this.dir = Math.cos(this.angle)
            this.dir2 = Math.sin(this.angle)
            this.angle += (Math.random()-.5)*.1
            if(zehwarldo.body.isPointInside(this.body)){
                this.body.x+=this.dir*this.speed
                this.body.y+=this.dir2*this.speed
            }else{
                this.angle += Math.PI
                this.body.x -= (this.link.hypotenuse()*(.001*this.speed))*Math.cos(this.link.angle())
                this.body.y -= (this.link.hypotenuse()*(.001*this.speed))*Math.sin(this.link.angle())
            }
        }
        draw(){
            if(this.artificial == 1){
                this.ai()

            for(let t = 0;t<this.links.length;t++){
                if(this.links[t].hypotenuse() < 500){
                    this.links[t].draw()
                }
            }
            }else{
                this.bodysto = {}
                this.bodysto.x = this.body.x
                this.bodysto.y = this.body.y
                if(zehwarldo.body.isPointInside(this.body)){
                ...
                // canvas_context.translate(this.bodysto.x-this.body.x, this.bodysto.y-this.body.y)
            }
            this.body.draw()
        }
    }

    let zehwarldo = new World()
    let ship = new Ship()
    for(let t = 0;t<15;t++){
        ship = new Ship()
        ship = new Ship()
    ...
    function main() {
        canvas_context.clearRect(-100000, -100000, canvas.width*199999, canvas.height*1999999)  // refreshes the image
